...
Export PDF: click the "Export PDF" button on any spec page
           (uses Chrome/Edge native print-to-PDF — zero extra dependencies)
"""
...
SCREEN_CSS = """
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .topbar svg  { width: 24px; height: 24px; flex-shrink: 0; }
    ...
    .topbar a:hover, .btn-pdf:hover { background: rgba(255,255,255,.2); }
    .btn-pdf { display: flex; align-items: center; gap: 6px; }

    /* ── Layout ── */
    .layout { display: flex; min-height: calc(100vh - 52px); }

    /* ── Sidebar ── */
    .sidebar { width: 230px; flex-shrink: 0; background: #fff;
    ...
    /* ── Content ── */
    .content { flex: 1; padding: 40px 56px 60px; max-width: 1060px; }
"""

CONTENT_CSS = """
    h1 { font-size: 26px; font-weight: 700; color: #172b4d;
         border-bottom: 2px solid #dfe1e6; padding-bottom: 14px; margin-bottom: 28px; }
    h2 { font-size: 18px; font-weight: 700; color: #172b4d;
         margin: 36px 0 12px; padding-top: 8px;
         border-left: 4px solid #0052cc; padding-left: 10px; }
    h3 { font-size: 14px; font-weight: 600; color: #172b4d; margin: 20px 0 8px; }
    p  { font-size: 14px; line-height: 1.7; color: #172b4d; margin-bottom: 12px; }
    ul, ol { font-size: 14px; line-height: 1.7; margin: 8px 0 12px 22px; }

    /* ── Tables ── */
    table { width: 100%; border-collapse: collapse; font-size: 13px; margin: 14px 0 20px; }
    thead th { background: #ebecf0; color: #172b4d; font-weight: 600;
               text-align: left; padding: 9px 12px;
               border: 1px solid #c1c7d0; white-space: nowrap; }
    ...
    blockquote { border-left: 3px solid #ffab00; background: #fffae6;
                 ...
                 border-radius: 0 4px 4px 0; font-size: 13px; color: #172b4d; }

    /* ── Code ── */
    code { background: #f4f5f7; border: 1px solid #dfe1e6; border-radius: 3px;
           font-family: "SFMono-Regular", Consolas, monospace;
           font-size: 12px; padding: 1px 5px; }
    pre  { background: #f4f5f7; border: 1px solid #dfe1e6; border-radius: 4px;
           padding: 12px 16px; margin: 10px 0; overflow-x: auto; }
    pre code { border: none; padding: 0; font-size: 12px; background: transparent; }
...
PRINT_CSS = """
    @media print {
      ...
      /* Full-width content, no background */
      body        { background: #fff; color: #000; font-size: 11pt; }
      .content    { padding: 0; max-width: 100%; }

      /* Force color printing (tables, badges, blockquotes) */
      * { -webkit-print-color-adjust: exact; print-color-adjust: exact; }

      /* Page header: title + date on every page */
      @page {
        size: A4;
        margin: 18mm 15mm 18mm 15mm;
        @top-left   { content: "AI Data Gov"; font-size: 8pt; color: #6b778c; }
        @top-right  { content: string(doc-title); font-size: 8pt; color: #6b778c; }
        @bottom-right { content: "Page " counter(page) " / " counter(pages);
                        font-size: 8pt; color: #6b778c; }
      }

      /* Capture title for @page header */
      h1 { string-set: doc-title content(); }

      /* Section breaks — each ## starts a new page */
      ...
      table   { page-break-inside: avoid; break-inside: avoid; font-size: 9pt; }
      ...
      h1 { font-size: 18pt; border-bottom: 2pt solid #dfe1e6; }
      h2 { font-size: 13pt; margin-top: 0; padding-top: 0; }
      h3 { font-size: 11pt; }
    }
"""
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .card-actions a {
      text-decoration: none; color: #0052cc; font-size: 13px; font-weight: 500;
      border: 1px solid #0052cc; border-radius: 3px; padding: 5px 14px;
      white-space: nowrap;
    }
    .card-actions a:hover { background: #0052cc; color: #fff; }
    .card-actions a.pdf   { color: #5243aa; border-color: #5243aa; }
    .card-actions a.pdf:hover { background: #5243aa; color: #fff; }
    ...
    <div class="card-list">
      ...
      <div class="card">
        ...
        <div class="card-actions">
          ...
          <a href="/print/{{ spec.filename }}" class="pdf" target="_blank">⬇ PDF</a>
        </div>
      </div>
      {% endfor %}
    </div>
...
<head>
  ...
  <title>{{ title }}</title>
  <style>
    """ + SCREEN_CSS + CONTENT_CSS + PRINT_CSS + """
  </style>
</head>
<body>
  <div class="topbar">
...
# Minimal print-optimised page (no sidebar, no topbar — just the content)
PRINT_TEMPLATE = """
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
           background: #fff; color: #172b4d; padding: 32px 48px; }

    /* Screen: show instructions banner */
    .print-banner {
      background: #0052cc; color: #fff; padding: 12px 20px; margin: -32px -48px 32px;
      display: flex; align-items: center; justify-content: space-between;
      font-size: 13px;
    }
    ...
    .print-banner button {
      background: #fff; color: #0052cc; border: none; border-radius: 3px;
      padding: 6px 16px; font-size: 13px; font-weight: 600; cursor: pointer;
    }
    .print-banner button:hover { background: #e8f0fe; }
    .print-banner a { color: rgba(255,255,255,.75); font-size: 12px; text-decoration: none; }
    .print-banner a:hover { color: #fff; }
    ...
    @media print {
      ...
      * { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
      ...
      @page {
        size: A4;
        margin: 18mm 15mm 18mm 15mm;
        @top-left   { content: "AI Data Gov — Data Flow Specification";
                      font-size: 8pt; color: #6b778c; }
        @top-right  { content: string(doc-title); font-size: 8pt; color: #6b778c; }
        @bottom-right { content: "Page " counter(page) " / " counter(pages);
                        font-size: 8pt; color: #6b778c; }
      ...
      h1 { string-set: doc-title content();
           font-size: 18pt; border-bottom: 2pt solid #dfe1e6; }
      h2 { page-break-before: always; break-before: page;
           font-size: 13pt; margin-top: 0; }
      h2:first-of-type { page-break-before: avoid; break-before: avoid; }
      table  { page-break-inside: avoid; break-inside: avoid; font-size: 9pt; }
      tr     { page-break-inside: avoid; break-inside: avoid; }
      thead  { display: table-header-group; }
...
<body>
  <div class="print-banner">
    <div>
      ...
      &nbsp;—&nbsp; File › Print &nbsp;·&nbsp; Destination: <em>Save as PDF</em>
      &nbsp;·&nbsp; Paper: A4 &nbsp;·&nbsp; Enable "Background graphics"
    </div>
    <div class="actions">
...
def _load_spec(filename: str):
    """Returns (md_text, path) or aborts."""
...
def print_view(filename: str):
    """Print-optimised page — opens print dialog automatically."""
    md_text = _load_spec(filename)
    return render_template_string(
        PRINT_TEMPLATE,
        title    = _spec_title(filename),
...
if __name__ == "__main__":
    ...
    print(f"  Specs directory →  {OUTPUT_DIR.resolve()}")
